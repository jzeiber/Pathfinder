#ifndef _pathfinder_astar_tile4dir_cached_
#define _pathfinder_astar_tile4dir_cached_
...
/**
	\file pathfinderastartile4dircached.h
*/
...
/**
	\brief A * path finder for 2d movement in 4 directions with caching
	
	This is an A * path finder for tile based games where movement is possible in 4 directions surrounding each tile.  Up, down, left, and right.  The movement costs from state to state are cached to maximize performance.  If state change costs can change frequently, you should use the non-cached version of this class.
...
private:
	// Can't be copied
	AStarTile4DirCached(const AStarTile4DirCached &rhs);
	AStarTile4DirCached &operator=(const AStarTile4DirCached &rhs);
...
AStarTile4DirCached<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::AStarTile4DirCached():m_mapwidth(0),m_cachestartindex(0),m_helperliststartindex(0)
{

}

template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
AStarTile4DirCached<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::~AStarTile4DirCached()
{
	ClearOpenList();
	ClearClosedList();
}
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
void AStarTile4DirCached<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::ClearCache()
{
	m_cache.clear();
	m_cachestartindex=0;	
}
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
void AStarTile4DirCached<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::ClearCachePosition(const std::pair<long,long> &pos)
{
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
void AStarTile4DirCached<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::ClearClosedList()
{
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
void AStarTile4DirCached<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::ClearOpenList()
{
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
const PathResult AStarTile4DirCached<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::FindPath(const std::pair<long,long> &start, const std::pair<long,long> &goal, std::vector<std::pair<long,long> > &finalpath, MOVECOST &movecost, DESTCOST &destinationcost)
{
	// clear the lists
	ClearOpenList();
	ClearClosedList();
	...
	while(m_openlist.size()>0)
	{
		...
		if(currentnode->m_node!=goal)
		{
			...
			else
			{
				...
				{
					xx=currentnode->m_node.first+(i%3)-1;
					yy=currentnode->m_node.second+(i/3)-1;
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
const PathResult AStarTile4DirCached<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::FindPath(const std::pair<long,long> &start, const std::pair<long,long> &goal, std::vector<std::pair<long,long> > &finalpath, MOVECOST &movecost, MOVEBLOCKED &moveblocked, DESTCOST &destinationcost)
{
	// clear the lists
	ClearOpenList();
	ClearClosedList();
...
#endif	// _pathfinder_astar_tile4dir_cached_
