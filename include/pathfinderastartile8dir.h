#ifndef _pathfinder_astar_tile8dir_
#define _pathfinder_astar_tile8dir_
...
/**
	\file pathfinderastartile8dir.h
*/
...
/**
	\brief A * path finder for 2d movement in 8 directions
...
private:
	// Can't be copied
	AStarTile8Dir(const AStarTile8Dir &rhs);
	AStarTile8Dir &operator=(const AStarTile8Dir &rhs);
...
AStarTile8Dir<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::AStarTile8Dir()
{
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
void AStarTile8Dir<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::ClearClosedList()
{
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
void AStarTile8Dir<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::ClearOpenList()
{
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
const PathResult AStarTile8Dir<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::FindPath(const std::pair<long,long> &start, const std::pair<long,long> &goal, std::vector<std::pair<long,long> > &finalpath, MOVECOST &movecost, DESTCOST &destinationcost)
{
	// clear the lists
	ClearOpenList();
	ClearClosedList();
			...
			for(yy=currentnode->m_node.second-1; yy<=currentnode->m_node.second+1; yy++)
			{
				for(xx=currentnode->m_node.first-1; xx<=currentnode->m_node.first+1; xx++)
				{
					if(xx!=currentnode->m_node.first || yy!=currentnode->m_node.second)
					{
						...
						if(m_helperlist[std::pair<long,long>(xx,yy)].m_onclosed==false)
						{

							// get the astarnode from the list - will be NULL if it doesn't already exist
							astaropen=m_helperlist[std::pair<long,long>(xx,yy)].m_node;
...
template <class MOVECOST, class DESTCOST, class MOVEBLOCKED, class COSTTYPE>
const PathResult AStarTile8Dir<MOVECOST,DESTCOST,MOVEBLOCKED,COSTTYPE>::FindPath(const std::pair<long,long> &start, const std::pair<long,long> &goal, std::vector<std::pair<long,long> > &finalpath, MOVECOST &movecost, MOVEBLOCKED &moveblocked, DESTCOST &destinationcost)
{
	// clear the lists
	ClearOpenList();
	ClearClosedList();
			...
			for(yy=currentnode->m_node.second-1; yy<=currentnode->m_node.second+1; yy++)
			{
				for(xx=currentnode->m_node.first-1; xx<=currentnode->m_node.first+1; xx++)
				{
					if(xx!=currentnode->m_node.first || yy!=currentnode->m_node.second)
					{
						...
						// ignore node if it is already on the closed list
						if(moveblocked(currentnode->m_node,std::pair<long,long>(xx,yy))==false && m_helperlist[std::pair<long,long>(xx,yy)].m_onclosed==false)
						{
